...
const rules = [
  ...
  'DOMAIN-SUFFIX,3.219.243.226,REJECT',
  'DOMAIN-SUFFIX,3.233.129.217,REJECT',
  'DOMAIN-SUFFIX,54.224.241.105,REJECT',
  'DOMAIN-SUFFIX,18.213.11.84,REJECT',
  'DOMAIN-SUFFIX,34.237.241.83,REJECT',
  'DOMAIN-SUFFIX,50.16.47.176,REJECT',
  'DOMAIN-SUFFIX,52.6.155.20,REJECT',
  'DOMAIN-SUFFIX,52.22.41.97,REJECT',
...
function main(config) {
  ...
  regionOptions.regions.forEach((region) => {
     ...
     * 自己改正则的话记得必须把倍率的number值提取出来
         /**
     * 提取倍率符合要求的代理节点
     * 当前逻辑：只保留倍率 < 2 的节点 (过滤掉 2倍及以上)
     */
    let proxies = config.proxies
      .filter((a) => {
        // 1. 兼容手机端的正则提取 (支持 x1.5, 1.5x, 1.5倍 等格式)
        const regex = /(?:x|X|✕|✖|⨉|倍|rate)\s*([0-9.]+)|([0-9.]+)\s*(?:x|X|✕|✖|⨉|倍|rate)/i;
        const match = regex.exec(a.name);

        // 2. 获取倍率数值
        let multiplier = 0;
        if (match) {
            // 提取到的数字
            multiplier = parseFloat(match[1] || match[2]);
        } else {
            // 如果节点没写倍率（例如“香港中转01”），默认视为 1倍
            multiplier = 1.0; 
        }
